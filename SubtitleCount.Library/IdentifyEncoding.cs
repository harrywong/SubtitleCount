...
namespace Microshaoft.Text
{
        ...
        internal static string[] nicename = new string[]
   {
    ...
    , "ISO 2022CN", "utf-8", "Unicode", "ASCII", "shift-jis", ""
   };
        ...
        public virtual string GetEncodingName(sbyte[] rawtext)
        {
            ...
            scores[9] = JISProbability(rawtext);
            scores[10] = 0;
        ...
        /// <param name="rawtext">要判断的 <see cref="sbyte"/> 字节数组</param>
        /// <returns>返回 0 至 100 之间的可能性</returns>
        internal virtual int JISProbability(sbyte[] rawtext)
        {
            int score = 0;
            int i, rawtextlen = 0;
            int goodbytes = 0, asciibytes = 0;
            ...
            for (i = 0; i < rawtextlen; )
            {
                ...
                else if (rawtext[i] >= -95 && rawtext[i] <= -33)
                {
                   // asciibytes++;
                   //goodbytes += 1;
                    i++;
                ...
                else if (rawtext[i] >= -127 && rawtext[i] <= -97 || rawtext[i] >= -32 && rawtext[i] <= -17)
                {
                    if (rawtext[i] >= 64 && rawtext[i] <= 126 || rawtext[i] >= -128 && rawtext[i] <= -4)
                        goodbytes += 2;
                    else
                        return 0;
                    i += 2;
                ...
                //“第二位字节\\”使用0×40-0×7E、0×80-0xFC (共188个)
                else if (rawtext[i] >= -16 && rawtext[i] <= -4)
                {
                    if (rawtext[i] >= 64 && rawtext[i] <= 126 || rawtext[i] >= -128 && rawtext[i] <= -4)
                        goodbytes += 2;
                    else
                ...
                else if (rawtext[i] == -3 || rawtext[i] == -2 || rawtext[i] == -1)
                {
                    return 0;
                }
                else
